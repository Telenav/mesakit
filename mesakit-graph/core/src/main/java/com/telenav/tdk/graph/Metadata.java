...
public class Metadata implements Named, AsIndentedString, KryoSerializable, Validatable
{
    ...
    {
        value = Strings.stripTrailing(value, ".world");
        ...
        if (matcher.matches())
        {
            var metadata = new Metadata()
                    ...
                    .withDataFormat(valueOf(matcher.group("format")))
            ...
            if (parts.length == 4 || parts.length == 5)
            {
                try
                {
                    final var metadata = new Metadata()
                            ...
                            .withDataFormat(valueOf(parts[2]))
                            .withName(parts[3]);
                    if (parts.length == 5)
                    {
    ...
    {
        return withNodeCount(nodeCount(REQUIRE_EXACT).add(that.nodeCount(REQUIRE_EXACT)))
                .withWayCount(wayCount(REQUIRE_EXACT).add(that.wayCount(REQUIRE_EXACT)))
                .withRelationCount(relationCount(REQUIRE_EXACT).add(that.relations))
                .withVertexCount(vertexCount(REQUIRE_EXACT).add(that.vertexCount(REQUIRE_EXACT)))
                .withEdgeCount(edgeCount(REQUIRE_EXACT).add(that.edgeCount(REQUIRE_EXACT)))
                .withForwardEdgeCount(forwardEdgeCount(REQUIRE_EXACT).add(that.forwardEdgeCount(REQUIRE_EXACT)))
                .withEdgeRelationCount(edgeRelationCount(REQUIRE_EXACT).add(that.edgeRelationCount(REQUIRE_EXACT)))
                .withShapePointCount(shapePointCount(REQUIRE_EXACT).add(that.shapePointCount(REQUIRE_EXACT)))
    ...
    {
        return new FileName(dataSupplier
                + "-" + dataSpecification
                + "-" + dataFormat
                + "-" + new FileName(name.replaceAll("-", "_")).normalized()
                + (dataVersion == null ? "" : "-" + dataVersion.toString())
    ...
    {
        ensure(validator(type).isValid(LOGGER));
        return this;
    }
    ...
    public Count edgeRelationCount(final CountType type)
    {
        if (type == ALLOW_ESTIMATE && edgeRelations == null && dataSize() != null)
        {
            ...
            return dataSize().percent(Percentage.of(0.25)).asEstimate().ceiling(3);
        }
    ...
    public Count graphElementCount(final CountType type)
    {
        ...
        {
            final var count = edgeCount(type).add(vertexCount(type)).add(edgeRelationCount(type));
    ...
    public Count nodeCount(final CountType type)
    {
        if (type == ALLOW_ESTIMATE && nodes == null && dataSize() != null)
        {
            // Empirical value from OSM data analysis
            // See https://spaces.telenav.com:8443/display/TT/TDK+-+Graph+-+Data+Specifications
            return dataSize().percent(Percentage.of(12.5)).asEstimate().ceiling(3);
        }
        return nodes;
    }
    ...
    public Count placeCount(final CountType type)
    {
        ...
        {
            return nodeCount(ALLOW_ESTIMATE).percent(Percentage.of(0.01)).asEstimate().ceiling(3);
        }
        return places;
    }
    ...
    public Count wayCount(final CountType type)
    {
        if (type == ALLOW_ESTIMATE && ways == null && dataSize() != null)
        {
            ...
            // See https://spaces.telenav.com:8443/display/TT/TDK+-+Graph+-+Data+Specifications
            return dataSize().percent(Percentage.of(0.5)).asEstimate().ceiling(3);
        }
        return ways;
    }
    ...
    private static Metadata extractPbfMetadata(final File file)
    {
        ...
        try
        {
            reader.process("Processing", new PbfDataProcessor()
            {
                @Override
    ...
    private static Metadata metadata(final PbfEntity<?> entity)
    {
        ...
        final var size = Bytes.parse(Listener.NULL, tags.get("telenav-data-size"));
