...
public abstract class Route implements Iterable<Edge>, Bounded, AsString
{
    ...
    public static class Converter extends BaseStringConverter<Route>
    {
        ...
        protected Route onConvertToObject(final String value)
        {
            ...
            {
                final var edges = new EdgeSet();
                for (final var edge : Strings.split(value, separators.current()))
                {
        ...
        {
            final var edges = new StringList(Maximum.of(route.size()));
    ...
    public static class MapIdentifierConverter extends BaseStringConverter<Route>
    {
        ...
        public MapIdentifierConverter(final Graph graph, final Separators separators, final Listener<Message> listener)
        {
            super(listener);
            this.graph = graph;
            this.separators = separators;
    ...
    private static class ConcatenatedRoute extends Route
    {
        ...
        public Iterator<Edge> iterator()
        {
            return new BaseIterator<>()
            {
                ...
                private final Stack<Route> stack = new Stack<>(Maximum.of(count()));
    ...
    {
        final var edges = new EdgeSet(Estimate.of(size()));
    ...
    {
        final var size = Estimate.of(this);
        ...
        array.compress(Compressible.Method.RESIZE);
        return array;
    }
    ...
    {
        return VertexSet.forIterable(Estimate.of(size() + 1), vertexes());
    }
    ...
    public boolean connectsTwoDoubleDigitizedRoads2()
    {
        ...
        if (polyline().isBent(Distance.MAXIMUM, Angle._90_DEGREES))
        {
            return false;
        }
    ...
    public boolean equals(final Object object)
    {
        if (object instanceof Route)
        {
            ...
            if (size() == that.size())
            {
                ...
                {
                    return Iterables.equals(this, that) && Objects.equal(startOffset, that.startOffset)
                            && Objects.equal(endOffset, that.endOffset);
                }
            }
        }
        return false;
    ...
    public boolean isLoop()
    {
        ...
        final var last = last();
        return first.from().equals(last.to()) && first.heading().isClose(last.heading(), Angle.degrees(20));
    }
    ...
    {
        return polyline().isStraight(Distance.MAXIMUM, Angle.degrees(10));
    }
    ...
    {
        final var last = new ObjectList<Edge>(Maximum.of(count));
    ...
    private boolean areOnSameLogicalWay(final Edge edgeIntoRoute, final Edge edgeOutOfRoute, final Route route)
    {
        ...
        return routeFirst.isOnSameWay(edgeIntoRoute) || routeLast.isOnSameWay(edgeOutOfRoute)
                || new EdgePair(edgeIntoRoute, routeFirst).isStraight(Angle.degrees(25D))
                || new EdgePair(routeLast, edgeOutOfRoute).isStraight(Angle.degrees(25D));
    }
    ...
    private boolean isAngleValid(final Edge edgeFrom, final Edge edgeTo)
    {
        ...
        if (headingFrom.difference(headingTo, Chirality.SMALLEST).isGreaterThan(Angle.degrees(30)))
        {
            return false;
        }
        ...
        return intersectionAngleFrom.isBetween(Angle.degrees(40), Angle.degrees(140), Chirality.SMALLEST)
                && intersectionAngleTo.isBetween(Angle.degrees(40), Angle.degrees(140), Chirality.SMALLEST);
    }
    ...
    private boolean isAttributeValid(final Edge edge)
    {
        ...
        // check bent
        return isFromVertexConnectedToRoute(edge) ? !edge.roadShape().isBent(Distance.meters(20), Angle.degrees(40)) : !edge.roadShape().reversed().isBent(Distance.meters(20), Angle.degrees(40));
    }
    ...
    private boolean isDoubleDigitizedWithDifferentNames(final EdgePair edgePair)
    {
        return hasTheSameRef(edgePair)
                && edgePair.isDoubleDigitized(EdgePair.DoubleDigitizedType.MISMATCHED_NAMES, Angle.degrees(35));
    }
