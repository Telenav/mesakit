...
public abstract class EdgeStore extends ArchivedGraphElementStore<Edge>
{
    ...
    public Count addAll(final EdgeSequence edges, final GraphConstraints constraints)
    {
        ...
        information("Added $ edges, discarded $ in $", Count.of(size()), discarded(), start.elapsedSince());
     ...
     */
    public boolean contains(final WayIdentifier identifier)
    {
     ...
     */
    public final NodeIdentifier retrieveFromNodeIdentifier(final Edge edge)
    {
        if (edge.isReverse())
        {
     ...
     * @see Speed
     */
    public Speed retrieveHistoricalSpeed(final SpeedPatternIdentifier identifier, final Speed reference,
    ...
    {
        return HOV_LANE_COUNT.retrieveObject(edge, Count::of);
    }
    ...
    {
        return LANE_COUNT.retrieveObject(edge, Count::of);
    }
    ...
    public Route retrieveRouteForWayIdentifier(final WayIdentifier wayIdentifier)
    {
     ...
     */
    public final NodeIdentifier retrieveToNodeIdentifier(final Edge edge)
    {
        if (edge.isForward())
        {
            return TO_NODE_IDENTIFIER.retrieveObject(edge, PbfNodeIdentifier::new);
    ...
    public synchronized void storeAttributes(final Edge edge)
    {
        ...
        if (laneCount != null)
        {
            LANE_COUNT.storeObject(edge, laneCount.minimum(Count.of(63)));
        }
