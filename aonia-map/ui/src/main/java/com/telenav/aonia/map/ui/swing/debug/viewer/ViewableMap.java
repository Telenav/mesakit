...
package com.telenav.aonia.map.ui.swing.debug.viewer;
...
import com.telenav.aonia.map.ui.swing.debug.DrawingSurface;
...
import com.telenav.aonia.map.ui.swing.debug.viewer.swing.SwingDrawingSurface;
...
public class ViewableMap
{
    private static class Entry
    {
        ...
        public void outline(final Shape outline)
        {
    ...
    {
        if (surface instanceof InteractiveDrawingSurface)
        {
            ((InteractiveDrawingSurface) surface).highlight(entry == selected);
        }

        // draw it
        entry.viewable.draw(surface);

        // and if it's being drawn on a Swing drawing surface,
        if (surface instanceof InteractiveDrawingSurface)
        {
            // get the shape outline that was drawn
            entry.outline(((SwingDrawingSurface) surface).hitTestShape());
        }
    }
    ...
    private List<Entry> entriesForPoint(final Point2D point)
    {
        ...
        synchronized (this)
        {
            ...
            {
                if (entry.outline != null && entry.outline.contains(point))
                {
                    entries.add(entry);
                }
            }
