...
package com.telenav.aonia.map.region.border.cache;
...
import com.telenav.aonia.map.data.formats.pbf.processing.readers.SerialPbfReader;
...
import com.telenav.aonia.map.region.RegionType;
import com.telenav.aonia.map.region.RegionType.RegionFactory;
import com.telenav.aonia.map.region.border.Border;
import com.telenav.aonia.map.region.border.BorderSpatialIndex;
...
import com.telenav.aonia.map.ui.swing.debug.debuggers.indexing.rtree.VisualRTreeSpatialIndexDebugger;
import com.telenav.aonia.map.ui.swing.debug.viewer.swing.SwingViewer;
...
import static com.telenav.aonia.map.data.formats.pbf.processing.PbfDataProcessor.Action.ACCEPTED;
...
/**
 * There should be no need for end-users of Aonia to use {@link BorderCache} objects since they are an implementation
 ...
 * queried. The border data can be found in the aonia-data project under the folder "boundaries". The borders can be
...
public abstract class BorderCache<T extends Region<T>> extends BaseRepeater
{
    ...
    private static final NetworkPath NETWORK_PATH = Host.parse("github.com").https().path(
            Message.format("Telenav/aonia-data/raw/master/administrative-borders/$/administrative-borders-$.jar",
    ...
    {
        return MapRegionProject.get().aoniaMapFolder().folder("region/borders");
    }
    ...
    private boolean loadBordersFromPbf(final Resource resource)
    {
        ...
        if (isDebugOn() && Booleans.isTrue(System.getProperty("AONIA_VISUAL_DEBUG")))
        {
